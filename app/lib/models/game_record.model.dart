...
class GameRecordWrapper {
  ...
  final GameRecordGroup data;

  GameRecordWrapper({required this.code, required this.status, required this.message, required this.data});

  factory GameRecordWrapper.fromRawJson(String str) => GameRecordWrapper.fromJson(json.decode(str));

  String toRawJson() => json.encode(toJson());

  factory GameRecordWrapper.fromJson(Map<String, dynamic> json) =>
      GameRecordWrapper(code: json["code"], status: json["status"], message: json["message"], data: GameRecordGroup.fromJson(json["data"] ?? {}));

  Map<String, dynamic> toJson() => {"code": code, "status": status, "message": message, "data": data.toJson()};
}

class GameRecordGroup {
  final List<GameRecord> list;
  final int count;
  ...
  factory GameRecordGroup.fromRawJson(String str) => GameRecordGroup.fromJson(json.decode(str));

  String toRawJson() => json.encode(toJson());

  factory GameRecordGroup.fromJson(Map<String, dynamic> json) => GameRecordGroup(
    list: List<GameRecord>.from((json["list"] ?? []).map((x) => GameRecord.fromJson(x))),
    count: json["count"] ?? 0,
...
class GameRecord {
  ...
  factory GameRecord.fromRawJson(String str) => GameRecord.fromJson(json.decode(str));
